...
class DroneDataFrame(tk.Frame):
    def __init__(self, root, uri, state, params, command_el=None, command_rl=None, indv_takeoff=None, indv_reset=None):
        super().__init__(root, background=dark2)

        self.command_el = command_el
        self.command_rl = command_rl
        self.indv_takeoff = indv_takeoff
        self.indv_reset = indv_reset
    ...
    def update(self, uri, state, params, msgs):
        ...
        self.indv_reset_button.command = lambda uri_i=uri.split("/")[-1]: self.indv_reset(uri_i)
